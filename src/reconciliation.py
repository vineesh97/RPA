...
def run_Reconciliation(start_date,end_date,service_name,df_excel):
    ...
    if service_name=='recharge':
        result = recharge_Service(start_date,end_date,df_excel)
    return result
...
def recharge_Service(start_date, end_date,df_excel):
...
def aeps_Service(start_date, end_date):
    logger.info("---Service mapping---")
    query = f'''
       select mt2.TransactionRefNum ,pat.requestID as vendor_reference ,mt.TransactionStatus  as Tenant_Status,mt2.TransactionStatus  as HUB_Master_status, 
        mst.TransactionStatus as MasterSubTrans_status,pat.TransStatus  as  Aeps_status
        from iHubTenantPortal_dev.MasterTransaction mt 
        left join ihub_dev.MasterTransaction mt2 
        on mt.Id =mt2.TenantMasterTransactionId 
        left join ihub_dev.MasterSubTransaction mst 
