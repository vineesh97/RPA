...
def run_Reconciliation(from_date, to_date, service_name, df_excel):
...
def filtering_data (df_db,df_excel):
   ...
   #VENDOR_SUCCESS_IHUB_FAILED___
    ...
    Vendor_success_ihub_initiated= mismatched[
            (mismatched['STATUS'] == 'Success') &  # STATUS in df_recharge_excel is Success
            (mismatched['Recharge_status'] == "success") &  # Recharge_status in df1 is Success
            (mismatched['HUB_Master_status'] == "initiated" )  # Mst_status in df1 is NOT Success
    ]
    Vendor_failed_ihub_initiated= mismatched[
            (mismatched['STATUS'] == 'failed') &  # STATUS in df_recharge_excel is Success
            (mismatched['Recharge_status'] == "success") &  # Recharge_status in df1 is Success
            (mismatched['HUB_Master_status'] == "initiated" )  # Mst_status in df1 is NOT Success
    ]
    Vendor_failed_ihub_success= mismatched[
            (mismatched['STATUS'] == 'failed') &  # STATUS in df_recharge_excel is Success
            (mismatched['Recharge_status'] == "success") &  # Recharge_status in df1 is Success
            (mismatched['HUB_Master_status'] == "success" )  # Mst_status in df1 is NOT Success
    ]
    #Vendor_success_ihub_inprogress= mismatched[
    #        (mismatched['STATUS'] == 'Success') &  # STATUS in df_recharge_excel is Success
    #        (mismatched['Recharge_status'] == "success") &  # Recharge_status in df1 is Success
     #       (mismatched['HUB_Master_status'] == "inprogress" )  # Mst_status in df1 is NOT Success
    #]
    logger.info("---Filteration Ends:")
    return {"status":"200","not_in_vendor": not_in_vendor, "not_in_Portal": not_in_Portal.head(100),"Vendor_success_ihub_inprogress":Vendor_success_ihub_inprogress 
            ,"Vendor_success_ihub_failed":Vendor_success_ihub_failed,"Vendor_success_ihub_inprogress":Vendor_success_ihub_inprogress,"Vendor_success_ihub_initiated":Vendor_success_ihub_initiated
            ,"Vendor_failed_ihub_initiated":Vendor_failed_ihub_initiated,"Vendor_failed_ihub_success":Vendor_failed_ihub_success}  
